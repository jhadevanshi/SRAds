...
async function seedData() {
  try {
    ...
    const bapunagarAdv = await pool.query(`
      INSERT INTO advertisers (company_name, owner_name, email, phone, address, city, latitude, longitude, status)
      VALUES (
    ...
    const bapAd = await pool.query(`
      INSERT INTO ads (title, advertiser_id, media_id, ad_type, play_duration, status)
      VALUES (
        'Buy 1 Get 1 Free Coffee', $1, 6, 'CAMPAIGN', 15, 'Active'
      ) RETURNING id
    `, [bapAdvId]);
    ...
    const navAd = await pool.query(`
      INSERT INTO ads (title, advertiser_id, media_id, ad_type, play_duration, status)
      VALUES (
        'Laptops at 40% Off', $1, 8, 'CAMPAIGN', 15, 'Active'
      ) RETURNING id
    `, [navAdvId]);
