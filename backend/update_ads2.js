...
const newQuery = `
      SELECT 
        ...
        -- Real spend from playback_logs (not wallet_transactions):
        COALESCE(p.total_spend, 0) AS total_spend,
        
        -- Today's plays and spend:
      ...
      -- Total plays and spend from playback_logs
      LEFT JOIN (
        SELECT ad_id, COUNT(*) as total_plays, SUM(amount) as total_spend
        FROM playback_logs
        GROUP BY ad_id
      ) p ON p.ad_id = a.id
...
const updated = content.replace(
  /SELECT \s*a\.id,\s*a\.title,\s*m\.media_type as type,[\s\S]*?ORDER BY a\.created_at DESC/m,
  newQuery.trim()
);
