...
async function run() {
  ...
  try {
    ...
    const mediaRes = await client.query(`
      SELECT id, title FROM media WHERE title ILIKE '%cycle%' LIMIT 1
    `);
    
    if (mediaRes.rows.length === 0) {
      console.log('Could not find media with title containing "cycle"!');
      return;
    }
    ...
    const adRes = await client.query(`
      ...
      RETURNING id
    `, ['Cycle Ad - Bopal Cafe General', advId, mediaId]);

    await client.query('COMMIT');
    console.log(`Successfully attached 'cycle' media to Bopal Cafe and Restro! (New Ad ID: ${adRes.rows[0].id})`);
  } catch (err) {
