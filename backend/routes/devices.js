...
// Log individual ad playback
router.post('/:id/playback', async (req, res) => {
  try {
    const { campaignId, adId, duration } = req.body;
    
    // Use device id 4 instead of req.params.id if adsd_id doesn't match, but the frontend should pass the correct ID
    // Actually, req.params.id should be the correct device ID from the frontend.
    
    if (!adId || !duration) {
      return res.status(400).json({ success: false, message: 'adId and duration are required' });
    }

    await pool.query(
      `INSERT INTO playback_logs (device_id, campaign_id, ad_id, duration) 
       VALUES ($1, $2, $3, $4)`,
      [req.params.id, campaignId || null, adId, duration]
    );

    res.json({ success: true, message: 'Playback logged' });
  } catch (error) {
    console.error('Log playback error:', error);
    res.status(500).json({ success: false, message: 'Server error' });
  }
});
